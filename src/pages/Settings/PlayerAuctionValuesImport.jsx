...
import {
  getAuctionValuesImportStatus,
  importPlayerAuctionValuesCsv,
      ...
      <table className="table table-striped table-hover">
        ...
        <tbody>
          {rows.map((row, rowIndex) => (
            <tr key={`auction-values-preview-${rowIndex}`}>
              {headers.map((header) => (
        ...
        <table className="table table-striped table-hover">
          ...
          <tbody>
            {skipped.map((item, index) => (
              <tr key={`auction-values-skipped-${index}`}>
...
const PlayerAuctionValuesImport = () => {
  ...
  const previewRows = useMemo(
    () =>
      csvRows
        .filter((row) => `${getCsvValue(row, ["Player"])}`.trim())
        .slice(0, 25),
    [csvRows]
  );
  const isLoading = status === "loading" || status === "processing";

  const loadAuctionValuesImportStatus = async () => {
    const auctionValuesStatus = await getAuctionValuesImportStatus();
    setImportStatus(auctionValuesStatus);
  };

  useEffect(() => {
    loadAuctionValuesImportStatus().catch((error) => {
      setMessage(`Loading auction values import status failed: ${error.message}`);
      setStatus("failed");
    });
  }, []);
  ...
  const handleUploadCsv = () => {
    if (!selectedFile) {
      setStatus("failed");
      setMessage("Choose an auction values CSV file first.");
      return;
    }
    ...
    Papa.parse(selectedFile, {
      ...
      complete: (parseResults) => {
        ...
        setCsvRows(rows);
        setStatus("idle");
        setMessage(`Loaded ${rows.length.toLocaleString()} auction rows.`);
      },
      error: (error) => {
        setStatus("failed");
  ...
  const handleProcessCsv = async () => {
    ...
    try {
      ...
      setMessage("Loading...");
      const importResults = await importPlayerAuctionValuesCsv(
        selectedFile,
        selectedYear,
        setMessage
      );
      ...
      await loadAuctionValuesImportStatus();
      setStatus("complete");
      setMessage(
  ...
  return (
    <div className="stats-import m-2 md:m-10 p-2 md:p-10 bg-white dark:text-gray-200 dark:bg-secondary-dark-bg rounded-3xl">
      <Header category="Import" title="Import Player Auction Values" />
...
export default PlayerAuctionValuesImport;
